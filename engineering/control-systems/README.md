...
<img src="13-graphing-02.gif" alt="tangent lines on a curve" />
...
<img src="Integral_example.svg" alt="area under the curve" />
...
1. This video shows how PID concepts can be applied to [self-driving cars](https://www.youtube.com/watch?v=4Y7zG48uHRo)
2. This video shows [an FLL team tuning](https://www.youtube.com/watch?v=Z-RWZTJI8sU) an EV3 robot line follower
3. This is the [fastest LEGO line following robot](https://www.youtube.com/watch?v=9YLxIUn7avY) I've ever seen. The video has almost no details about how he did it, but this is useful to understand what's possible.
4. This video shows a [college lecture on tuning a PID control](https://www.youtube.com/watch?v=uXnDwojRb1g).
5. This is a simple demonstration of [tuning a PID with a robot arm](https://www.youtube.com/watch?v=fusr9eTceEo). It does a great job of showing what the different terms do.
6. This [StackExchange Q&A](https://robotics.stackexchange.com/questions/167/what-are-good-strategies-for-tuning-pid-loops) discusses mothods for tuning a PID.
7. This [forum discussion](https://forum.pololu.com/t/questions-about-pid-actually-pd-tuning/3263) talks about how to tune a PD line controller. They're working with robots that aren't EV3s, but most of the same techniques still apply.
8. If you have a good way of logging error and output values on each iteration of a loop, you might be able to use [this tool](https://pidtuner.com/) to help you tune a controller.
9. This is an [academic paper](https://www.researchgate.net/publication/283347335_Optimization_of_PID_Control_for_High_Speed_Line_Tracking_Robots) about going beyond PIDs to handle special cases that arise with line followers. The math formulas are advanced, but the implementation of the ideas need not be that complicated.
...
13. This writeup gets into how to apply the Zeigler-Nichols method to [estimate the I and D values](https://fll-pigeons.github.io/gamechangers/gyro_pid.html) on an EV3 robot. Hint: use a smartphone BPM app to figure out the oscillation rate.
14. This page goes into more detail on the [Zeigler-Nichols method](http://www.inpharmix.com/jps/PID_Controller_For_Lego_Mindstorms_Robots.html), including a version for when you don't want an integral term. See the section Tuning a PID Controller Without Complex Math
15. This book, [Feedback Systems](https://www2.econ.iastate.edu/tesfatsi/feedbacksystems.astrommurray.pdf), gives pseudocode on page 312 that implements a PID with integral anti-windup and derivative low-pass filtering with very simple math
